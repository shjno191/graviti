...
const TABS_CONFIG = [
    ...
    { id: 'compare-suite', label: '📊 Compare Suite', keywords: ['compare', 'database', 'diff', 'lab', 'schema', 'text', '📊'], type: 'main' },
    { id: 'translate', label: '🇯🇵 Translate', keywords: ['translate', 'jap', 'dictionary', 'quick', '🇯🇵'], type: 'main' },
...
function App() {
    ...
    const [suggestions, setSuggestions] = useState<typeof TABS_CONFIG>([]);
    const [selectedSugIdx, setSelectedSugIdx] = useState(-1);

    const handleTabJump = (sug: any) => {
        setActiveTab(sug.parent || sug.id);
        if (sug.type === 'sub') {
            if (sug.parent === 'compare-suite') setCompareSubTab(sug.subId);
            if (sug.parent === 'translate') setTranslateSubTab(sug.subId);
        }
        setGlobalSearchTerm('');
        setSuggestions([]);
        setSelectedSugIdx(-1);
    ...
    useEffect(() => {
        ...
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            if (combo === focusSearchShortcut.toUpperCase()) {
                e.preventDefault();
    ...
    }, [focusSearchShortcut, activeTab, compareSubTab, translateSubTab]);
    ...
    useEffect(() => {
        ...
        const matches = TABS_CONFIG.filter(t =>
        ...
        setSelectedSugIdx(matches.length > 0 ? 0 : -1);
    }, [globalSearchTerm]);
            ...
            <div className="flex items-center border-b border-gray-200 bg-white sticky top-0 z-[100] shadow-sm px-5 mt-3 mx-5 rounded-xl border">
                ...
                <div className="relative mr-4">
                    <input
                        ...
                        className="pl-8 pr-8 py-2 bg-gray-100 border border-transparent rounded-2xl text-xs focus:ring-2 focus:ring-primary w-64 focus:w-80 transition-all font-bold focus:bg-white focus:border-gray-200"
                    ...
                    {suggestions.length > 0 && (
                        <div className="absolute top-[calc(100%+8px)] right-0 w-80 bg-white border border-gray-200 rounded-2xl shadow-2xl z-[1000] overflow-hidden animate-in fade-in zoom-in-95 duration-200 flex flex-col p-1.5 backdrop-blur-xl bg-white/95">
                                        ...
                                        <div className="flex flex-col">
                                            ...
                                            {sug.type === 'sub' && (
                                                <span className={clsx("text-[8px] font-bold opacity-60 uppercase", selectedSugIdx === idx ? "text-white" : "text-primary")}>
                                                    Part of {sug.parent.split('-').map((w: string) => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')}
                                                </span>
                                            )}
                                        </div>
                                    </div>
